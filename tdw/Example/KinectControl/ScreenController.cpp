...
ScreenController::ScreenController( kvs::ScreenBase* screen, kvs::tdw::MasterMessageSender* sender ) :
    m_ref_screen( NULL ),
    m_ref_sender( sender )
{
    this->initialize();
    ...
    const kvs::Vector3f& center )
{
    ...
    m_ref_screen->mouse()->setMode( kvs::Mouse::Rotation );
    m_ref_screen->mouse()->setRotation( q );
    m_ref_sender->sendPaintEvent( m_ref_screen );
}
    ...
    const kvs::Vector3f& cur_hand )
{
    ...
    m_ref_screen->mouse()->setMode( kvs::Mouse::Translation );
    m_ref_screen->mouse()->setTranslation( t );
    m_ref_sender->sendPaintEvent( m_ref_screen );
}
    ...
    const kvs::Vector3f& cur_right_hand )
{
    ...
    m_ref_screen->mouse()->setMode( kvs::Mouse::Scaling );
    m_ref_screen->mouse()->setScaling( s );
    m_ref_sender->sendPaintEvent( m_ref_screen );
}
