...
void StochasticPointRenderer::calculate_zooming_factor( void )
{
    ...
    kvs::tdw::Camera* camera = new kvs::tdw::Camera();
...
void StochasticPointRenderer::setup_shader( const float modelview_matrix[16] )
{
    ...
    if ( kvs::tdw::Application::IsRenderer() )
    {
        ...
        ratio = ( aspect >= 1.0f ) ? full_size.x() / 512.0f : full_size.y() / 512.0f;
    }

    const float* mat = modelview_matrix;
    const float width = ratio * static_cast<float>( m_height );
